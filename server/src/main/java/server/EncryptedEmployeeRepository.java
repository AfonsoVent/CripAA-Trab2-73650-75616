...
public class EncryptedEmployeeRepository {
    ...
    public BigInteger calculatePayrollSumPaillier(String deptDet, BigInteger paillierNSquare) throws SQLException {
        ...
        try (Connection conn = DriverManager.getConnection(DatabaseConfig.URL, DatabaseConfig.USER, DatabaseConfig.PASS);
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    if (sSum != null) {
                        encryptedSum = encryptedSum.multiply(new BigInteger(sSum)).mod(paillierNSquare);
                    }
                }
            }
        }
    ...
    public Map<String, BigInteger> calculateBonusAllEligiblePaillier(String bonusDetEligible, long scalarModifier, BigInteger paillierNSquare) throws SQLException {
        String sql = "SELECT idDet, salarySum FROM " + DatabaseSchema.TABLE_NAME + " WHERE bonusDet = ?";
        ...
        try (Connection conn = DriverManager.getConnection(DatabaseConfig.URL, DatabaseConfig.USER, DatabaseConfig.PASS);
            ...
            try (ResultSet rs = ps.executeQuery()) {
                while (rs.next()) {
                    ...
                    String sSum = rs.getString("salarySum");
                    if (sSum != null) {
                        BigInteger salaryCiphertext = new BigInteger(sSum);
                        BigInteger bonusCiphertext = salaryCiphertext.modPow(scalar, paillierNSquare);
                        bonusMap.put(id, bonusCiphertext);
                    }
                }
            }
        }
